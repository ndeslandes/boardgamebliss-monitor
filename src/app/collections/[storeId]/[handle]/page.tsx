...
export default function CollectionPage({ params }: { params: { storeId: string; handle: string } }) {
  ...
  const displayed = useMemo(() => {
    ...
    else if (filter === 'wishlist') list = list.filter(p => wishlist.has(`${storeId}:${p.handle}`));
    ...
    return [...list].sort((a, b) => {
      if (sortBy === 'wishlist') {
        const aw = wishlist.has(`${storeId}:${a.handle}`);
        const bw = wishlist.has(`${storeId}:${b.handle}`);
  ...
  const stats = useMemo(() => {
    ...
    return { avail, oos: products.length - avail, total: products.length, wishlisted: [...wishlist].filter(k => k.startsWith(storeId + ':')).length };
  }, [products, wishlist, storeId]);
    ...
    <main className="min-h-screen bg-gray-950 text-gray-100">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          ...
          {domain && (
            <a
              href={`${domain}/collections/${handle}`}
              target="_blank"
              rel="noreferrer"
        ...
        {!products && !error && <p className="text-gray-600 text-sm">Loading {handle}…</p>}
                  ...
                  return [
                    ...
                    >
                            ...
                            {product.tags.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                {product.tags.slice(0, 5).map(tag => (
                                  <span key={tag} className="px-1.5 py-0.5 text-xs bg-gray-800 text-gray-500 rounded">{tag}</span>
                                ))}
                                {product.tags.length > 5 && <span className="text-xs text-gray-700">+{product.tags.length - 5}</span>}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="px-4 py-3 text-right text-gray-300 whitespace-nowrap">
                        {multiVariant ? (
                          <span className="text-gray-500 text-xs">from ${lowestPrice(product).toFixed(2)}</span>
                        ) : (
                          <>
                    ...
                    multiVariant && product.variants.map((v: CachedVariant) => (
                      <tr key={v.id} className={`bg-gray-950/60 text-xs ${!v.available ? 'opacity-40' : ''}`}>
                        ...
                        <td className="px-4 py-2 text-right">
                          <span className={v.available ? 'text-emerald-400' : 'text-gray-600'}>{v.available ? 'in stock' : 'sold out'}</span>
                        </td>
                      </tr>
                    )),
                  ];
